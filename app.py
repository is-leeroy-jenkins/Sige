...
def throw_if(name: str, value: Any) -> None:
    """
    Purpose:
        Raise ValueError if the value is None, empty, or otherwise invalid for a required parameter.
...
def to_long_format(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    id_candidates = ["AgencyName", "BureauName", "AccountName", "FiscalYear"]
    ...
    df_long["FiscalYear"] = df_long["FiscalYear"].astype(str).str.replace("FY", "", regex=False).astype(int)
...
def fit_predict_regressions(
    ...
    """
    Purpose:
        Train multiple regression models on (FiscalYear -> Outlays) and forecast future years.
    ...
    df_hist = df_grouped[(df_grouped["FiscalYear"] >= cfg.min_fy) & (df_grouped["FiscalYear"] <= cfg.max_fy)].copy()
    ...
    metrics_df = pd.DataFrame(rows_metrics).sort_values("R2", ascending=False).reset_index(drop=True)
...
def fit_predict_time_series(
    ...
    """
    Purpose:
        Fit ARIMA and Holt-Winters on the historical aggregated outlays series and forecast future years.
...
def main() -> None:
    ...
    st.caption("Upload historical outlays (FY columns), explore trends, and compare forecasting models.")
    ...
    with st.sidebar:
        ...
        fut_1 = st.number_input("Future FY #1", min_value=1900, max_value=2100, value=2025, step=1)
        fut_2 = st.number_input("Future FY #2", min_value=1900, max_value=2100, value=2026, step=1)
        ...
        seasonal_periods = st.number_input("Holt-Winters seasonal_periods", min_value=1, max_value=20, value=5, step=1)
    ...
    if "AgencyName" in df_long.columns:
        agencies = sorted([a for a in df_long["AgencyName"].dropna().unique().tolist() if str(a).strip() != ""])
    ...
    tab_overview, tab_models, tab_download = st.tabs(["Overview", "Models", "Export"])
    ...
    with tab_overview:
        ...
        with right:
            fig = plot_series(
                ...
                title="Total Outlays by Fiscal Year" if agency is None else f"Outlays by Fiscal Year — {agency}",
            )
    ...
    with tab_models:
        ...
        hist = df_grouped[(df_grouped["FiscalYear"] >= cfg.min_fy) & (df_grouped["FiscalYear"] <= cfg.max_fy)].copy()

        fig2 = plt.figure()
        plt.plot(hist["FiscalYear"], hist["Outlays"], label="Historical")

        fut_years = list(cfg.future_years)
        plt.plot(fut_years, forecast_df[top_model].to_numpy(), marker="o", label=f"Top Regression: {top_model}")
        plt.plot(fut_years, df_ts["ARIMA"].to_numpy(), marker="o", label="ARIMA")
        plt.plot(fut_years, df_ts["Holt-Winters"].to_numpy(), marker="o", label="Holt-Winters")
